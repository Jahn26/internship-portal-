...
const ApplicantDashboardPage = () => {
  ...
  useEffect(() => {
    api.get('/dashboard/applicant').then(({ data }) => setStats(data)).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div>
        <p className="text-sm uppercase tracking-[0.25em] text-teal-300">Applicant</p>
      ...
      <section className="space-y-3">
        ...
        )) : <EmptyState title="No applications yet" description="Browse jobs and submit your first application." />}
      </section>
    </div>
  );
};
